...
test.group('Api career subcategory request list', () => {
  const baseUrl = "/client/career-subcategory"
  test('subcategory index data test', async ({ client }) => {
  ...
  test('subcategory show data test', async ({ client }) => {
    const user = await Client.find(1)
    const id = 101
    ...
    response.assertBody({
      ...
      data: {
        id: data.data.id,
        career_category_id: data.data.career_category_id,
        ...
        category: {
          ...
          created_by: data.data.category.created_by,
          icon: data.data.category.icon,
          name: data.data.category.name,
          created_at: data.data.category.created_at,
          updated_at: data.data.category.updated_at,
        }
      },
      msg: data.msg
    })
